...
import { isOperationalReductionFilter, OPERATIONAL_REDUCTION_FILTERS } from '../dashboardDrilldown'
...
export function TransactionsPage() {
  ...
  function updateEconomicScope(value: string) {
    const next = new URLSearchParams(params)
    if (value === 'operational_reductions') {
      for (const [key, filterValue] of Object.entries(OPERATIONAL_REDUCTION_FILTERS)) next.set(key, filterValue)
    } else {
      next.delete('operational_economic_only')
      next.delete('reduction_only')
    }
    next.set('offset', '0')
  ...
  return <>
    ...
    <Card sx={{ p: 2, mb: 2 }}><Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: 'repeat(3, 1fr)', xl: 'repeat(6, 1fr)' }, gap: 2 }}>
      ...
      <FormControl size="small"><InputLabel>Ambito</InputLabel><Select label="Ambito" value={isOperationalReductionFilter(params) ? 'operational_reductions' : 'all'} onChange={(e) => updateEconomicScope(String(e.target.value))}><MenuItem value="all">Tutte le transazioni</MenuItem><MenuItem value="operational_reductions">Riduzioni antifrode operative</MenuItem></Select></FormControl>
