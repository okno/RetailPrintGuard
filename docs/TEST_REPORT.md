...
| `.venv\\Scripts\\python -m pytest -q -p no:cacheprovider` sul monorepo `0.1.5` prima della pubblicazione | Windows, CPython 3.12.13 | `93 passed, 14 skipped`, 1 warning Starlette/httpx; include la regressione del contratto TAB per le directory dispositivo |
